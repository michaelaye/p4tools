...
src="https://img.shields.io/pypi/v/p4tools.svg" /></a>
...
## Raw classification → catalog pipeline (v4.0+)

The full path from a raw Zooniverse / Panoptes data export to the final
fan/blotch catalog passes through three modules in this order:

    panoptes_extract  →  production.cleaning  →  production.{dbscan,fnotching,catalog}
       (raw CSV)         (canonicalisation)        (the actual reduction)

1.  **`p4tools.panoptes_extract.extract_panoptes_classifications(csv)`**
    — parses the per-classification annotations JSON from a Panoptes
    `planet-four-classifications.csv` (workflow 12978), enriches with
    metadata + tile coords + image URLs, and writes a flat parquet
    (`panoptes_workflow_12978.parquet`).

2.  **`p4tools.production.cleaning.clean_classifications(df, source=...)`**
    — *automatically called* by
    [`extract_panoptes_classifications`](https://michaelaye.github.io/p4tools/panoptes_extract.html#extract_panoptes_classifications)
    for the Panoptes path, but also exposed as a standalone step for the
    legacy `zooniverse_v1` interface. Drops dirt and out-of-frame
    markings, canonicalises blotch and fan angles, and adds the
    `x_angle` / `y_angle` features the clusterer reads.

3.  **`p4tools.production.{dbscan, fnotching, catalog}`** — the catalog
    reduction proper: DBSCAN clustering (L0 → L1A), fan/blotch fnotching
    (L1A → L1B), and cut-threshold catalog production (L1B → L1C). These
    modules are unchanged from v3.x.

Older v3.x catalogs were produced from a queryable parquet that already
had cleaning baked in by the legacy `planet4.reduction` CLI. v4.0 makes
the cleaning step a first-class p4tools module so the full pipeline is
reproducible from any raw source.
