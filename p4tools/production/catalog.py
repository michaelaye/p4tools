...
# %% auto #0
__all__ = ['LOGGER', 'MIN_CLUSTER_SIZE', 'execute_in_parallel', 'fan_id_generator', 'blotch_id_generator', 'get_L1A_paths',
...
# %% ../../notebooks/05_production.catalog.ipynb #18dddbfb
# other imports
...
# %% ../../notebooks/05_production.catalog.ipynb #a5fdaf81
#starting the logger
LOGGER: Logger = logging.getLogger(__name__)

# %% ../../notebooks/05_production.catalog.ipynb #ef19ac67
...
# %% ../../notebooks/05_production.catalog.ipynb #cb35c99a
from typing import Any, Generator
...
# %% ../../notebooks/05_production.catalog.ipynb #014de9c5
...
def cluster_obsid(obsid=None, savedir=None, imgid=None, dbname=None, min_cluster_size=MIN_CLUSTER_SIZE):
    """Cluster all image_ids for given obsid (=image_name) with post-clustering validation.

    Validates that all clusters have at least `min_cluster_size` members,
    matching the DBSCAN min_samples requirement that can be violated during
    multi-stage clustering (XY → radii → angles).
    ...
    min_cluster_size : int, optional
        Minimum number of votes required for a cluster to be included in the catalog.
        Default is 3, matching DBSCAN's min_samples requirement.
    """
    ...
    dbscanner = dbscan.DBScanner(savedir=savedir, dbname=dbname, save_results=False)
        ...
        if kind in dbscanner.reduced_data:
            ...
            for df in dbscanner.reduced_data[kind]:
                if df is not None and len(df) > 0:
                    # n_votes column contains cluster member count
                    valid_df = df[df['n_votes'] >= min_cluster_size]
                    rejected = len(df) - len(valid_df)
                    if rejected > 0:
                        logger.info("Rejected %d %s clusters with < %d votes for %s",
                                   rejected, kind, min_cluster_size, obsid)
                        total_rejected += rejected
                    validated.append(valid_df)
            dbscanner.reduced_data[kind] = validated

    if total_rejected > 0:
        logger.info("Total rejected clusters for %s: %d", obsid, total_rejected)
...
# %% ../../notebooks/05_production.catalog.ipynb #8728c8c5
...
# %% ../../notebooks/05_production.catalog.ipynb #b79af207
...
# %% ../../notebooks/05_production.catalog.ipynb #f3b506f1
...
# %% ../../notebooks/05_production.catalog.ipynb #00fea0ef
...
# %% ../../notebooks/05_production.catalog.ipynb #104cef13
