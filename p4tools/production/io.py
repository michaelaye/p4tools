...
# %% ../../notebooks/05a_production.io.ipynb #d19752b3
...
# %% auto #0
__all__ = ['LOGGER', 'pkg_name', 'configpath', 'data_root', 'get_config', 'set_database_path', 'get_data_root',
           'get_ground_projection_root', 'resolve_dbname', 'get_db_stats', 'get_random_tile', 'check_and_pad_id',
           'PathManager', 'DBManager']

# %% ../../notebooks/05a_production.io.ipynb #488669d4
...
# %% ../../notebooks/05a_production.io.ipynb #e981ae39
from configparser import ConfigParser
from pathlib import Path
import warnings
...
def get_config() -> ConfigParser | None:
    ...
    ConfigParser or None
        Dictionary with the content of the configpath file, or None if config
        file does not exist.
    """
    if not configpath.exists():
        warnings.warn(
            f"Config file {configpath} not found. "
            f"Run p4tools.production.io.set_database_path('/path/to/data') to set up.",
            stacklevel=2,
        )
        return None
...
def set_database_path(dbfolder) -> None:
    ...
    if d is None:
        d = configparser.ConfigParser()
        d["planet4_db"] = {}
    d["planet4_db"]["path"] = str(dbfolder)
...
# %% ../../notebooks/05a_production.io.ipynb #9p5crhej9ss
def resolve_dbname(db: str | None = None) -> str:
    """Resolve the database path from an explicit argument or the config file.

    Parameters
    ----------
    db : str, optional
        Explicit path to the Parquet database.  If *None*, the path is read
    ...
    ------
    ValueError
        If *db* is None and no ``dbname`` is stored in the config.
    """
    if db is not None:
        return str(Path(db).expanduser().resolve())
    config = get_config()
    try:
        return config["planet4_db"]["dbname"]
    except KeyError:
        raise ValueError(
            "No database path given and none stored in config. "
            "Use: p4 setup <root> --db <path>"
        )


def get_db_stats(db: str | None = None, top: int = 10) -> dict:
    """Compute summary statistics for a raw Planet Four database.

    Parameters
    ----------
    db : str, optional
        Path to Parquet database. Resolved from config if *None*.
    top : int
        Number of top tiles/obsids to include.

    Returns
    -------
    dict
        Keys: ``n_markings``, ``n_tiles``, ``n_obsids``,
        ``avg_per_tile``, ``avg_per_obsid``,
        ``type_counts`` (dict), ``top_tiles`` (DataFrame),
        ``top_obsids`` (DataFrame).
    """
    db = resolve_dbname(db)
    df = pd.read_parquet(db, columns=["image_id", "image_name", "marking"])

    n_total = len(df)
    n_tiles = df.image_id.nunique()
    n_obsids = df.image_name.nunique()

    # marking type breakdown
    type_counts = df.marking.value_counts().to_dict()

    # top tiles
    tile_counts = df.groupby("image_id").size().rename("markings")
    tile_obsid = (
    ...
    # top obsids
    obsid_counts = df.groupby("image_name").size().rename("markings")
    obsid_ntiles = df.groupby("image_name")["image_id"].nunique().rename("tiles")
    top_obsids = (
        obsid_counts.nlargest(top)
        .to_frame()
        .join(obsid_ntiles)
        .reset_index()
    ...
    return {
        ...
        "n_tiles": n_tiles,
        "n_obsids": n_obsids,
        "avg_per_tile": n_total / n_tiles,
        "avg_per_obsid": n_total / n_obsids,
        "type_counts": type_counts,
        "top_tiles": top_tiles,
        "top_obsids": top_obsids,
    }


def get_random_tile(db: str | None = None, tolerance: float = 0.5) -> dict:
    ...
    db : str, optional
        Path to Parquet database. Resolved from config if *None*.
    tolerance : float
        Fractional tolerance around the mean (default 0.5 = within 50%).

    Returns
    -------
    dict
        Keys: ``image_id``, ``image_name``, ``markings``, ``avg``.
    """
    db = resolve_dbname(db)
    df = pd.read_parquet(db, columns=["image_id", "image_name"])
    tile_counts = df.groupby("image_id").size()
    avg = tile_counts.mean()
    lo, hi = avg * (1 - tolerance), avg * (1 + tolerance)

    candidates = tile_counts[(tile_counts >= lo) & (tile_counts <= hi)]
    pick = candidates.sample(1)
    tile_id = pick.index[0]
    count = int(pick.iloc[0])
    obsid = df[df.image_id == tile_id].image_name.iloc[0]
...
# %% ../../notebooks/05a_production.io.ipynb #6be00e15
...
# %% ../../notebooks/05a_production.io.ipynb #68b12995
class PathManager:
    ...
    * L1C : Derived database where a cut has been applied for fnotches to become either fan or
    ...
    def clustering_logfile(self):
        """
        Returns the path to the clustering settings JSON file.
        This method constructs the path to the "clustering_settings.json" file
        located in the same directory as the fanfile.
        Returns
        -------
        pathlib.Path
            The path to the "clustering_settings.json" file.
        """

        return self.fanfile.parent / "clustering_settings.json"
...
# %% ../../notebooks/05a_production.io.ipynb #ea0a053d
class DBManager:
    ...
    ----------
    dbname : str, optional
        Path to database file to be used. Must be provided explicitly.
    ...
    def __init__(self, dbname=None, obsid = None):
        ...
        ----------
        dbname : <str>
            Filename of database file to use.
        """
        if dbname is None:
            raise ValueError(
                "dbname must be provided. "
                "Pass the path to a database file (e.g., a .parquet file)."
            )
